"""
Secondary Network Stack
Creates the network infrastructure in the secondary region.
"""
...
class SecondaryNetworkStack(Stack):
    """
    Stack that creates the network infrastructure for the secondary region.
    ...
    def _create_vpc(self) -> None:
        """Create the VPC with mirrored subnet configuration."""

        # Get configuration
        vpc_cidr = self._config.get("vpc_cidr", "10.1.0.0/16")
        availability_zones = self._config.get("availability_zones", 2)
        nat_gateways = self._config.get(
            "nat_gateways", 1
        )  # Reduced for cost in secondary

        # Create VPC with mirrored subnet configuration
        self._vpc = ec2.Vpc(
            self,
            "VPC",
    ...
    def _create_security_groups(self) -> None:
        ...
        self._alb_security_group = ec2.SecurityGroup(
            ...
            "ALBSecurityGroup",
            vpc=self._vpc,
            description="Security group for Application Load Balancer (Secondary)",
        ...
        self._ecs_security_group = ec2.SecurityGroup(
            ...
            "ECSSecurityGroup",
            vpc=self._vpc,
            description="Security group for ECS tasks (Secondary)",
            allow_all_outbound=True,
        )
        ...
        self._database_security_group = ec2.SecurityGroup(
            ...
            vpc=self._vpc,
            description="Security group for RDS database (Secondary)",
        ...
        self._vpc_endpoint_security_group = ec2.SecurityGroup(
            ...
            vpc=self._vpc,
            description="Security group for VPC endpoints (Secondary)",
            allow_all_outbound=False,
        )
